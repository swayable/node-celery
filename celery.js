...
function Configuration(options) {
  ...
  self.TASK_QUEUES = self.TASK_QUEUES || null;
...
function RedisBackend(conf) {
  ...
  // redis 라이브러리로 auth하려면 auth() 함수를 사용하면 안 되고,
  // url 형식으로 넣어줘야 한다
  // --> node-celery 소스를 직접 수정하고 프로젝트 내부로 코드 이동
  debug('Connecting to backend...');
  // self.redis = redis.createClient(purl.port, purl.hostname);
  self.redis = redis.createClient(purl.href);
  ...
  self.backend.on('ready', function() {
    ...
    if (self.conf.broker_type === 'redis') {
      ...
      self.broker = amqp.createConnection(self.conf.BROKER_OPTIONS, {
        ...
        const logQueues = (self.conf.TASK_QUEUES||[]);
        console.log(logQueues)
        if (logQueues){
          setTimeout(()=>{
            logQueues.forEach(lq=>{
              const queueName = `${self.conf.BROKER_PREFIX?`__${self.conf.BROKER_PREFIX}__`:""}${lq.queue || lq}`;
              self.broker.queue(queueName, {durable:true, autoDelete:false}, (q)=>{
                // q.bind(exchange, lq.routing_key || "*");
              });
            })
          }, 2000);
        }
    ...
    self.broker.on('error', function(err) {
      self.emit('error', err);
      console.error("broker conn error::");
    });
...
function Task(client, name, options, exchange) {
  ...
  self.publish = function (args, kwargs, options, callback) {
    ...
    queue = options.queue || self.options.queue || queue || self.client.conf.TASK_QUEUES || self.client.conf.DEFAULT_QUEUE;
    ...
    if (exchange) {
      ...
      const exec = q =>{
        const queueName = `${self.client.conf.BROKER_PREFIX?`__${self.client.conf.BROKER_PREFIX}__`:""}${q.queue || q}`;
        console.log(`self.client.broker.publish(${queueName}, msg=${msg}, pubOptions, callback);`);
        self.client.broker.publish(queueName, msg, pubOptions, callback);
      };
